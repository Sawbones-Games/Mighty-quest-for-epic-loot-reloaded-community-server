...
// Stateful account model. The GetAccountInformation body is generated from this. State accrues as the
// player progresses: boot (no hero/castle) → pick+name castle (empty) → ChooseFirstHero (no gear/spells)
// → forest dungeon (gold + first loot) → equip loot → store (buy+equip weapon) → witch dungeon → skill
// earned at level 2. Mirrors MQELOffline_cpp Source/Backend (Wallet/Hero).
class AccountState
{
    public long AccountId { get; set; } = 3123971;          // test account id
    ...
    // Wallet (Backend::Wallet — amount + capacity per currency). InGameCoin starts 1000 (DEFAULTACCOUNT),
    // LifeForce starts 0. Capacities are the rank-1 GoldStorage/LifeForceStorage spec values (see
    // WalletCapacityUpdated).
    ...
    // 0-based (RenovationLevel1=0 .. RenovationComplete=4 per CastleRenovationCatalog). Advanced and persisted
    ...
    // emits these as CompletedAssignments so the client skips done steps on reconnect instead of restarting. A List
    // (not a set) to keep completion order. Deduped on insert.
   ...
   public List<Objective> Objectives { get; set; } = new();
    ...
    // sums the loot tables it sent in the matching StartAttack to score the gold/lifeforce/xp and hand back the
    // looted items. This combat scratch is transient — held in a per-account in-memory cache (so it survives the
    ...
    // Per-class first-loot TemplateId the forest trap drops — class-appropriate gear the hero can equip.
    // Archer 81 = padded cloth tunic. Knight 17 / Mage 53 / Runaway 311.
    ...
    // A new hero (Backend::Hero::Createhero + HeroService::ChooseFirstHero): Level 1, XP 0, one consumable
    // (template 1), AttackRegion 1 unlocked, no spells, plus the class's basic starter gear (default weapon +
    // armor). A hero with no weapon crashes the client. The forest dungeon then drops the first looted item on
    // top of this set. Gear IDs from the reference.
    public HeroState CreateHero(int cls)
    {
        ...
        switch (cls)
        {
            ...
            case 5: // Runaway — real starter items unknown; the Knight set lets the hero be created but is a
                    // known-incomplete stand-in. TODO: find real items.
                Eq("MainHand", 2, 108); Eq("Head", 8, 109); Eq("Body", 8, 110); Eq("Hands", 8, 111); Eq("Shoulders", 8, 132); break;
        }
        Hero = h;
        return Hero;
    ...
    // Generate the AccountInformation Result from the clean first-run base plus real state.
    // firstRunResult = the "Result" object of account-information-firstrun.json (static boilerplate +
    // minimal empty castle + in-game-coin wallet).
  ...
  res["Stats"] = new JsonObject();
            // Objectives (campaign quests). The engine only tracks and completes an objective whose state is
            // seeded in this GAI account model: a runtime UnlockObjectiveAssignmentActionSpec alone lists it in
            // the UI but its conditions never commit (all stay Status 0), so ObjectiveCompletedAssignmentTriggerSpec
            // never fires and 005006 never starts. So the unlocked objectives must be seeded here from real state
            // (gameState.Objectives, populated by ObjectiveUnlockCommand and persisted).
            // AccountObjective shape (from the reference ObjectiveUnlockedNotification capture):
            ...
            // Crafting materials are delivered in-session (client-driven), not via a GAI seed field: the account
            // model is frozen at boot, so a GAI field only updates on reconnect.
            // MQELOffline_cpp serves CompletedAssignments as a flat int[] in the hero-created body; the client
            // reads it on boot to skip finished FTUE steps. Empty here = the client restarts the tutorial.
    ...
    // Objective completion is engine-side: the assignment VM unlocks the objective and the engine completes it
    // from its conditions, firing ObjectiveCompletedAssignmentTriggerSpec → the next assignment (005006). The
    // server owes nothing at completion.
}

// A single hero (the chosen class). Gear / Spells / Xp / Level accrue as the player plays.
class HeroState
{
    ...
    // required to be level L: L1=0, L2=75, L3=400, L4=1000, …
    ...
    // a threshold). Returns the new total XP.
    ...
    // Numeric HeroEquipmentEquipCommand.DestinationSlot -> named equipment slot (the keys Gear uses):
    // 3=Body, 8=MainHand. An unknown slot returns null (the equip is skipped and logged) rather than guess.
    // Extend as more slots are observed on the wire.
